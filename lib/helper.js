...
class Helper {
    constructor(configs) {
        this.getDocumentClient = () => {
            if (!this.configs || !this.configs.options)
                return client_1.documentClient();
            return client_1.documentClient(this.configs.options);
        };
        this.getClient = () => {
            if (!this.configs || !this.configs.options)
                return client_1.client();
            return client_1.client(this.configs.options);
        };
        /** Get */
        ...
        if (configs) {
            config_1.config.update(configs);
        }
    }
}
